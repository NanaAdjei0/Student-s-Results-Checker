...
<?php
    ...
    // Retrieve sub_name from the URL
    $sub_name = isset($_GET['sub_name']) ? $_GET['sub_name'] : "";

   
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // ... Your code to handle form submission and database updates ...
...
$term = $_GET['term'];
        ...
        $statusMsg = "Scores saved successfully!"; // You can modify this message as needed
    }
?>
...
<head>
    ...
    <!-- Include necessary libraries: jQuery and DataTables CSS/JS -->
    <!-- ... -->
    <!-- Include your custom JavaScript file for AJAX handling -->

    <script>
        // Function to submit the form data using AJAX
        function submitForm() {
            var formData = $('#scoreForm').serialize(); // Serialize the form data
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Display the status message and form on the page
                    $('#statusMsg').html(response);
                }
            });
        }
    </script>
    <script src="your_custom_script.js">
...
<!-- SIDE BAR NAVIGATION BAR -->
 ...
 <main>
 <h2 style="margin-bottom: 2vh;">Compute Results/ Exercise 1</h2>
 <?php
include('dbconnection.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($scores as $index => $score) {
        ...
        if (!empty($studentName) && is_numeric($score)) {
            ...
            // Check if a score for this student, sub_name, and classroom already exists for the given academic year and term
            $checkQuery = "SELECT * FROM tblresult WHERE name = '$studentName' AND sub_name = '$sub_name' AND class_id = '$classroom' AND yearid = '$academic' AND termid = '$term'";
            $checkResult = mysqli_query($con, $checkQuery);

            if (mysqli_num_rows($checkResult) > 0) {
                // If a record exists, update the score
                $updateQuery = "UPDATE tblresult SET exx1 = '$score' WHERE name = '$studentName' AND sub_name = '$sub_name' AND class_id = '$classroom' AND yearid = '$academic' AND termid = '$term'";
                ...
                $insertQuery = "INSERT INTO tblresult (name, exx1, sub_name, class_id, stu_index, yearid, termid) VALUES ('$studentName', '$score', '$sub_name', '$classroom', '$studentStuIndex', '$academic', '$term')";
...
// Function to get the score for a given student, sub_name, and classroom
function getStudentScore($con, $studentName, $sub_name, $classroom) {
    $query = "SELECT exx1 FROM tblresult WHERE name = '$studentName' AND sub_name = '$sub_name' AND class_id = '$classroom'";
...
<form id="scoreForm" method="post" style="width: 100%;">
        <input type="hidden" name="sub_name" value="<?php echo htmlspecialchars($sub_name); ?>">
...
<table id="studentTable">
    <tbody>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $studentScore = isset($scoresMap[$studentName]) ? htmlspecialchars($scoresMap[$studentName]) : "";
...
<!-- using a ajax library to maintain forms even when user press on submit to let user be on the same page, include (dbconnection.php)  declare variables that is passed from the user , both from the form and the one passed by the url parameter (sub_name, classroom, class_id) , use this query to display students based on the selection by user in the previous form($query = "
...
$result = mysqli_query($con, $query);
) retrieve all students names based on that sort written by the code above , beside each name should be an input field to enter score(exx1) into the column named exx1 in the database table called tblresult ; these input should be saved together with name (as in name of the student) in the column called name and sub_name should be get as passed by the url parameter to be saved in the column called sub_name and also classroom should be saved in the column called class_id , Nb: input can also be updated by user anytime  -->
