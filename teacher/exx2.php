...
<!-- SIDE BAR NAVIGATION BAR -->
 ...
 <main>
 <h2 style="margin-bottom: 2vh;">Compute Results/ Exercise 2</h2>
<?php
  ...
  function getStudentScore($con, $studentName, $sub_name, $classroom) {
      $query = "SELECT exx2 FROM tblresult WHERE name = '$studentName' AND sub_name = '$sub_name' AND class_id = '$classroom'";
      ...
      foreach ($scores as $index => $score) {
          ...
          if (!empty($studentName) && is_numeric($score)) {
              ...
              if (mysqli_num_rows($checkResult) > 0) {
                  ...
                  $updateQuery = "UPDATE tblresult SET exx2 = '$score' WHERE name = '$studentName' AND stu_index = '$studentStuIndex' AND sub_name = '$sub_name' AND class_id = '$classroom' AND yearid = '$academic' AND termid = '$term'";
                  mysqli_query($con, $updateQuery);
                    // Provide a response message for updated score
                $statusMsg = "Score for $studentName updated successfully.";
              } else {
                  // If no record exists, insert a new record
                  $insertQuery = "INSERT INTO tblresult (name, stu_index, exx2, sub_name, class_id , yearid, termid) VALUES ('$studentName', '$studentStuIndex', '$score', '$sub_name', '$classroom', '$academic', '$term')";
