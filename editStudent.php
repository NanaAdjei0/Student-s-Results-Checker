...
<?php
    ...
    if (isset($_POST['update'])) {
        ...
        $name = $_POST['name'];
        $stuIndex = $_POST['stu_index'];
        $classId = $_POST['class'];
    
        // Update student information in the database
        $updateQuery = "UPDATE students SET name = '$name', stu_index = '$stuIndex', class = '$classId' WHERE stu_index = '$studentIndex'";

        $result = mysqli_query($con, $updateQuery);
    
        if ($result) {
            // Redirect back to the student list page after successful update
            header("Location: viewStudent.php"); // Replace with the actual URL of your student list page
            exit();
        } else {
...
if (isset($_GET['editStudentId'])) {
    $studentIndex = $_GET['editStudentId'];

    // Retrieve student information based on the student index
    $query = "SELECT name, stu_index, class FROM students WHERE stu_index = '$studentIndex'";
    ...
    if ($result) {
        ...
        if (!$row) {
            ...
            // Display the student information in an edit form
            ?>
            <h1>Edit Student</h1>
            <form action="" method="POST">
                ...
                <input type="text" name="stu_index" value="<?php echo $row['stu_index']; ?>"><br>
                <label for="class">Class:</label>
                <input type="text" name="class" value="<?php echo $row['class']; ?>"><br>

                <input type="submit" name="update" class="gradient-button" style="margin-bottom: 2vh; margin-top: 2vh; color:rgb(6, 6, 37); text-decoration: underline; font-family: 'Poppins', sans-serif;" value="Update">
            </form>
            <?php
        }
    } else {
